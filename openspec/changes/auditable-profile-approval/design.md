...
- `PublicEventProjection` (D6) carries no centre identity at all, so there is nothing for the audit data to attach to (the D10 non-correlation assessment is unchanged). *(Historical: events DO name their hosting centre since the later `events-show-centre` revision. The conclusion here is unaffected — the audit fields still attach to nothing, because `ProfileReview` is never joined into either public read path — but the stated premise no longer holds. See `docs/security-threat-model.md`.)*
