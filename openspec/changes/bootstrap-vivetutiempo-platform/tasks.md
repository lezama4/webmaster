...
- [x] 4.1 Prisma schema additions (B3 pr2-review — BLOCKER, FIRST, no migration yet): add `ProfileStatus.DEACTIVATED`; add `Profile.reviewRequestedAt: DateTime?`; add a `Session` model (`id`, `accountId` FK + account-lookup index, `tokenHash`, `absoluteExpiresAt`, `lastActiveAt`, `createdAt`) linked to `Account`. Edits `prisma/schema.prisma` only (ADR D8).
- [x] 4.2 Base Prisma migration: `prisma/migrations/.../migration.sql` — ALL base tables/FKs (Account/Profile/Slot/Proposal/Event) INCLUDING the 4.1 additions and the `Session` table + its index. Order: schema additions + base tables/FKs + Session table/index FIRST — strictly before task 4.4's raw-SQL migration (B3/N2, pr2-review).
- [x] 4.3 [Integration test] `tests/integration/schema-migration.test.ts`: apply the base migration (4.2) against an empty database; assert success and that all expected tables/columns/indexes (incl. `Session`, `reviewRequestedAt`, `DEACTIVATED`) exist (B3/N2, pr2-review).
- [x] 4.4 Raw-SQL migration (B1 pr2-review — BLOCKER, applied strictly after 4.2/4.3): partial unique indexes on `"proposals"` against the Prisma-generated identifiers — `CREATE UNIQUE INDEX ... ON "proposals"("slotId") WHERE "status" = 'ACCEPTED'::"ProposalStatus"` and `CREATE UNIQUE INDEX ... ON "proposals"("slotId","artistProfileId") WHERE "status" = 'SUBMITTED'::"ProposalStatus"` (see design.md "Migration / Rollout" for the exact SQL).
- [x] 4.5 [Integration test] `tests/integration/partial-index-catalog.test.ts`: query `pg_indexes` after migration and assert both partial indexes exist with the exact predicates in 4.4 (B1, pr2-review).
- [x] 4.6 Prisma repositories in `src/infrastructure/persistence/prisma/` (Account/Profile/Slot/Proposal/Event).
- [x] 4.7 `PublicEventProjectionQuery` Prisma implementation (M6 pr2-review): the ONLY adapter permitted to join Event→Slot→Proposal→Profile; returns the finished `PublicEventProjection` shape, filtered to published Events.
- [x] 4.8 [Integration test] `tests/integration/public-event-projection-query.test.ts`: asserts the query returns only allow-listed fields against fixture/seeded data, filtered to published Events (M6 pr2-review; the HTTP-level no-leak test stays PR 3/6).
- [x] 4.9 `MatchingUnitOfWork` with `withLockedSlot(slotId, work)` (B2 pr2-review, ADR D4): `SELECT ... FOR UPDATE` on the Slot row FIRST, then loads the live Slot + full Proposal set inside the same transaction, invokes the `work` callback, persists its result before commit. Used by `submitProposal`, `approveProposal`, `rejectProposal`, `closeSlot`.
- [x] 4.10 [Integration test] `tests/integration/matching-race.test.ts`: two parallel `approveProposal` calls on the same Slot -> exactly one `accepted`, one `409 ConflictError` (Docker Postgres).
- [x] 4.11 [Integration test] `tests/integration/submit-approve-race.test.ts`: barrier-based interleave of `submitProposal` and `approveProposal` on the same Slot — approval locks first; assert the late `submitProposal` is rejected once the Slot is `filled` (Docker Postgres) (B1/B2, pr2-review).
- [x] 4.12 [Integration test] `tests/integration/submit-close-race.test.ts`: barrier-based interleave of `submitProposal` and `closeSlot` on the same Slot — assert the late `submitProposal` is rejected once the Slot is `closed` (M2 pr2-review).
- [x] 4.13 [Integration test] `tests/integration/approve-close-race.test.ts`: barrier-based interleave of `approveProposal` and `closeSlot` targeting the same Slot — assert exactly one coherent outcome persists, never a contradictory success response from both (M2 pr2-review).
- [x] 4.14 [Integration test] `tests/integration/approve-reject-race.test.ts`: barrier-based interleave of `approveProposal` and `rejectProposal` targeting the same Proposal — assert one coherent serial outcome, no contradictory success response (M1 pr2-review).
- [x] 4.15 [Integration test] `tests/integration/close-reject-race.test.ts`: barrier-based interleave of `closeSlot` and `rejectProposal` targeting a Proposal on the same Slot — assert one coherent serial outcome (M1 pr2-review).
- [x] 4.16 [Integration test] `tests/integration/duplicate-submission.test.ts`: two concurrent `submitProposal` calls by the SAME Artist against the SAME open Slot — assert exactly one `submitted` Proposal persists and the second attempt is denied with `ConflictError`, never a raw DB constraint error surfacing to the caller (M2 DECISION, pr2-review).
- [x] 4.17 [Integration test] `tests/integration/close-slot-cascade.test.ts`: closing a Slot with `submitted` Proposals persists the cascade atomically — Slot `closed`, every `submitted` Proposal `rejected` (Docker Postgres) (B2, pr2-review).
- [x] 4.18 `ProfileUnitOfWork` with `withLockedProfile(accountId, work)` (M3, D7 — mirrors 4.9's `withLockedSlot`): locks the Profile/Account row FIRST, loads the live Profile inside that lock, then lets `work` transition status + call `SessionPort` operations, persisting before commit. Used by `deactivateProfile`, `validateProfile` (reject branch), `login`.
- [x] 4.19 DB-backed `SessionPort` adapter: `src/infrastructure/auth/session.ts` implementing `create`/`resolveValid`/`touch`/`revokeOne`/`revokeAllForAccount` against the `Session` table (httpOnly/Secure/SameSite=Lax cookie carries only the opaque token; the row stores its hash) (M3, D7, D8).
- [x] 4.20 [Integration test] `tests/integration/session-lifecycle.test.ts`: absolute expiry rejected, idle expiry rejected, rotation issues a new id on login, logout deletes the row (M3).
- [x] 4.21 [Integration test] `tests/integration/profile-transition-session-revocation.test.ts`: Admin deactivation and Admin rejection each atomically revoke all sessions for the Account via `ProfileUnitOfWork` (reject-revocation and deactivation-revocation cases, M3).
- [x] 4.22 [Integration test] `tests/integration/login-vs-deactivation-race.test.ts`: barrier-based interleave of `login` and `deactivateProfile` for the same Account — assert the login is denied once the deactivation commits (M3).
- [x] 4.23 argon2id `PasswordHasher`: `src/infrastructure/auth/passwordHasher.ts`.
- [x] 4.24 `Clock`/`IdGenerator` adapters.
- [x] 4.25 Postgres-backed `LoginRateLimiter` adapter: `src/infrastructure/auth/loginRateLimiter.ts` — atomic counter/window per account + per hashed/truncated IP, rolling-window cleanup policy, generic response for unknown vs. locked-out accounts (M4 pr2-review — distinct from the pre-existing M4 "public projection allow-list" finding, ADR D7).
- [x] 4.26 [Integration test] `tests/integration/login-rate-limiter.test.ts`: attempts within the window count correctly, the window resets, expired windows are cleaned up, response shape is identical for unknown vs. locked-out accounts (M4 pr2-review).
...
- [x] 4.28 [Test] `tests/unit/infrastructure/csrf.test.ts`: allowed origin passes; a hostile `Host` header does not bypass the check; cross-site `Origin` is rejected; absent headers are rejected; the `login` route is covered by the same check (M5 pr2-review).

**Verification note (PR 2b session):** all Phase 4 code and tests are implemented and type-check/lint clean. The 15 integration test files above are written against a REAL Postgres instance (no mocks) but could NOT be executed in this session — Docker Desktop's Linux backend cannot start because Windows virtualization/WSL2 is disabled on this machine (`wsl --status` confirms; `docker ps` fails to reach the engine). Every integration test file guards itself with `describe.skipIf(!await isDatabaseAvailable())` so `npm run test` reports them as SKIPPED, never a false pass. Re-running with a working Docker/Postgres (or CI) is required to obtain real execution evidence before this phase can be considered fully verified.

**Remediation note (PR 2b adversarial-review fixes, this session):** `reviews/codex-pr2b-review.md`'s confirmed findings are fixed: pr2b-B1 (BLOCKER, login rate limiter now atomic via one `INSERT ... ON CONFLICT ... DO UPDATE ... RETURNING` `consumeAttempt` call, replacing the racy `isAllowed`+`recordFailure` split — required a flagged, minimal `LoginRateLimiter`/`login.ts` touch), pr2b-M1 (`SessionPort.touch` now conditional on BOTH absolute and idle validity, zero-row update = unauthenticated, stale row deleted — required a flagged, additive `touch(): Promise<boolean>` port signature widening; nothing in `application/` consumed the old `Promise<void>` return, so no use case changed), pr2b-M2 (Argon2id params pinned explicitly to the OWASP baseline — memoryCost 19456, timeCost 2, parallelism 1, version, output length — plus an upgrade-on-successful-login path via a new `PasswordHasher.needsRehash` port method), pr2b-M3 (Vitest now runs `tests/unit/**` and `tests/integration/**` as separate projects; the integration project is `fileParallelism: false` and applies every migration via a `globalSetup` before any test file loads), pr2b-M5 (every previously-unforced race — approve/approve, approve/close, approve/reject, close/reject, duplicate-submit — now uses the `afterLock` barrier in BOTH lock orderings, and `login-vs-deactivation-race.test.ts` now covers the login-locks-first ordering too), pr2b-N1 (partial-index catalog tests now assert `CREATE UNIQUE INDEX`, exact key order, `pg_index.indpred IS NOT NULL`, plus a duplicate-insert behavioural proof for both indexes), pr2b-N2 (rate-limiter key is now a server-keyed HMAC-SHA256, not a bare SHA-256, with rotation/retention documented in `loginRateLimiter.ts`), pr2b-N3 (new test proves only `tokenHash` is persisted and neither the row id nor the hash itself can authenticate). pr2b-M4 (CSRF route wiring) is explicitly out of scope here — it remains Phase 5/PR 3. `npm run test` -> 269 passed, 49 skipped (Postgres unreachable locally, same root cause as above); `npx tsc --noEmit` -> 0 errors; `npm run lint` -> 0 problems.
...
**Read-side query layer for the role dashboards (this session, PR 3 slice — strict TDD, application layer only, in support of 5.3/5.4/5.6/5.10/5.12's still-pending `page.tsx` UI work):** built the two dedicated read-model verticals the pending UI pages need, mirroring `OpenSlotListingQuery`/`listOpenSlots` and `PublicEventProjectionQuery`/`listPublishedEvents`'s shape exactly.
- `src/application/dto/PendingProfileView.ts` + `src/application/ports/PendingProfileQuery.ts` + `src/application/use-cases/listPendingProfiles.ts` (Admin pending-profile queue, 5.3/5.12): admin-only (`assertRole(actor, "admin")`), returns every `pending`-status Profile — INCLUDES `rejected -> pending` re-registrations, same queue, no separate path (5.12) — ordered oldest-first by `requestedAt` (`reviewRequestedAt` if set, else `createdAt`; adapter's responsibility). Allow-list: `profileId`, `type`, `displayName`, `requestedAt` — no `accountId`, email, or password hash.
- `src/application/dto/HospitalSlotView.ts` (+ nested `HospitalSlotProposalView`) + `src/application/ports/HospitalSlotBoardQuery.ts` + `src/application/use-cases/listHospitalSlots.ts` (Hospital's own slot board, 5.4/5.6/5.10): hospital-only, live-active-profile-gated (mirrors `listOpenSlots`'s N2/pr2a-N1 pattern), scoped to ONE Hospital's own Slots via `activeProfile.id`, ordered by `scheduledAt`. Read-only listing — no lock needed; the mutating use cases already re-check live status/ownership at decision time. Allow-list: `slotId`, `title`, `scheduledAt`, `status`, `proposals[].{proposalId, artistDisplayName, message, status}` — no `hospitalProfileId`/`artistProfileId`/email.
- Both use cases rebuild a FRESH DTO field-by-field (mirrors `listPublishedEvents`'s D6/pr2a-B1 discipline) — a hostile/widened adapter cannot leak an extra property through the use-case boundary; each has a dedicated "HOSTILE ADAPTER" unit test proving it.
- Prisma adapters `src/infrastructure/persistence/prisma/{PendingProfileQuery,HospitalSlotBoardQuery}.ts`: limited `select` (never `include`), fresh DTO construction, same discipline as `PrismaPublicEventProjectionQuery`.
- `src/infrastructure/composition/container.ts` extended with `pendingProfilesDeps()`/`hospitalSlotBoardDeps()`, reusing the singleton `prismaClient` and the new adapters — no existing adapter touched.
- Artist proposals read-model (optional per this task's scope) was SKIPPED to keep this slice bounded — the Artist page can rely on the existing `listOpenSlots` for the MVP; a dedicated `listArtistProposals` is left for whichever slice builds the Artist UI page.
- Strict TDD followed for every use case: RED (`tests/unit/application/{listPendingProfiles,listHospitalSlots}.test.ts` written first, confirmed failing on missing module) -> GREEN (use case implemented, tests pass) -> REFACTOR (none needed — first-pass shape matched the mirrored pattern).
- Verified: `npx prisma generate` OK; `npx tsc --noEmit` 0 errors; `npm run lint` 0 problems; `npm run test` -> 281 passed (269 pre-existing + 12 new), 55 skipped (Postgres unreachable locally, same pre-existing cause as every prior batch).
- No route handlers, UI pages, or auth code touched — read-side application/infrastructure layer only, per explicit scope for this slice. The `page.tsx` files for `admin/profiles`, `hospital/slots`, and `artist/slots` (5.3/5.4/5.5's remaining note) still need to be built on top of these use cases plus route handlers exposing them (not yet added).
